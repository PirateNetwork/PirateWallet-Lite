...
Remove-Item -Force -ErrorAction Ignore ./artifacts/linux-binaries-zer-qt-wallet-v$version.tar.gz
Remove-Item -Force -ErrorAction Ignore ./artifacts/linux-deb-zer-qt-wallet-v$version.deb
Remove-Item -Force -ErrorAction Ignore ./artifacts/Windows-binaries-zer-qt-wallet-v$version.zip
Remove-Item -Force -ErrorAction Ignore ./artifacts/Windows-installer-zer-qt-wallet-v$version.msi
Remove-Item -Force -ErrorAction Ignore ./artifacts/macOS-zer-qt-wallet-v$version.dmg
...
scp -r src/ res/ ./zer-qt-wallet.pro ./application.qrc ./LICENSE ./README.md ${server}:/tmp/zqwbuild/ | Out-Null
...
if (! (Test-Path ./artifacts/linux-binaries-zer-qt-wallet-v$version.tar.gz) -or
    ! (Test-Path ./artifacts/linux-deb-zer-qt-wallet-v$version.deb) -or
    ! (Test-Path ./artifacts/Windows-binaries-zer-qt-wallet-v$version.zip) -or
    ! (Test-Path ./artifacts/Windows-installer-zer-qt-wallet-v$version.msi) ) {
        Write-Host "[Error]"
        exit 1;
    }
Write-Host "[OK]"
